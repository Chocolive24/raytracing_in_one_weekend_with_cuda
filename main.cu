...
__global__ void Render(Vec3F* fb, Camera** camera, Hittable** world,
  ...
  for (int s_j = 0; s_j < (*camera)->sqrt_spp; s_j++) {
    for (int s_i = 0; s_i < (*camera)->sqrt_spp; s_i++) {
      const RayF r((*camera)->GetRayAtLocation(i, j, s_i, s_j, &local_rand_state));
      col += (*camera)->CalculatePixelColor(r, world, &local_rand_state);
    }
  }
...
__global__ void FinalScene(Camera** d_camera, Hittable** d_list,
  ...
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    ...
    *d_world = new BVH_Node(d_list, 0, object_idx - 1, rand_state);
    *d_camera = new Camera();
    (*d_camera)->Initialize();
  }
}
...
__global__ void StratifiedCornell(Camera** d_camera, Hittable** d_list,
                                 Hittable** d_world, curandState* rand_state) {
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    curandState local_rand_state = *rand_state;
    ...
    d_list[2] = new Quad(Vec3F(343, 554, 332), Vec3F(-130, 0, 0),
                         Vec3F(0, 0, -105), light);
    d_list[3] =
        new Quad(Vec3F(0, 0, 0), Vec3F(555, 0, 0), Vec3F(0, 0, 555), white);
    d_list[4] = new Quad(Vec3F(555, 555, 555), Vec3F(-555, 0, 0),
                         Vec3F(0, 0, -555), white);
    d_list[5] =
        new Quad(Vec3F(0, 0, 555), Vec3F(555, 0, 0), Vec3F(0, 555, 0), white);
...
int main() {
  ...
  case 11: {
      StratifiedCornell<<<1, 1>>>(d_camera, d_list, d_world, d_rand_state2);
      break;
    }
    default:
      break;
