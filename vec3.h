...
public:
  ...
  __host__ __device__ [[nodiscard]] constexpr Vec3<T> Refract(
      const Vec3<T>& n, 
      const T& etai_over_etat) const noexcept {
    const auto cos_theta = std::fmin(-(*this).DotProduct(n), 1.0);
    const auto r_out_perp = etai_over_etat * (*this + cos_theta * n);
    const auto r_out_parallel = -std::sqrt(std::fabs(1.0 - r_out_perp.LengthSquared())) * n;
